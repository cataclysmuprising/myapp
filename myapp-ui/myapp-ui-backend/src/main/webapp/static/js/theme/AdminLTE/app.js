...
function loadValidationErrors() {
  if ($("#validationErrors .error-item").length > 0) {
    ...
    $.each($("#validationErrors .error-item"), function(index, item) {
      var elementId = $(item).attr("data-id").replace(new RegExp("\\.", "g"), '_');
      if ($("#" + elementId).length) {
    ...
    $.each($('.nav-tabs-custom .tab-pane'), function(index, item) {
      if ($(this).find(".has-error").length > 0) {
        if (!$(this).hasClass("active")) {
          $('.nav-tabs > li > a[href="#' + $(this).attr("id") + '"]').trigger("click");
        }
        return false;
      }
    });
  ...
  if ($.fn.DataTable) {
    $.extend(true, $.fn.dataTable.defaults, {
      ...
      footerCallback: function(roles, start, end, max, total, pre) {
        $('table.dataTable').off('processing.dt').on('processing.dt', function(e, settings, processing) {
          if (processing) {
            Pace.start();
          } else {
            Pace.stop();
...
function initDatePicker() {
  if ($.fn.datetimepicker) {
    $(".input-group.date").each(function(index, item) {
      var format = $(this).attr("data-format");
      $(this).datetimepicker({
        format: format == undefined || format.length == 0 ? 'DD-MM-YYYY' : format,
        showClear: true
      });
    });
  }
  ...
  if ($.fn.validate) {
    $.validator.setDefaults({
      ...
      errorPlacement: function(error, element) {
        if (element.parent('.input-group').length) {
          ...
          if ((element).closest('.form-group .input-group').length) {
            error.insertAfter(element.parent().closest('.form-group .input-group'));
          } else {
            error.insertAfter(element.parent().closest('.bootstrap-select'));
          }
        } else if (element.prop('type') === 'radio') {
          if (element.parent().closest('.radio-inline-group')) {
            error.appendTo(element.parent().closest('.radio-inline-group')).wrap('<div/>');
          }
        } else {
      ...
      highlight: function(element) {
        ...
        if (inputElemCounts > 1 && $(container).find(".radio-inline-group").length == 0) {
...
function isNotEmpty(pValue) {
  if (null == pValue) {
    return false;
  } else if (pValue.toString() != "") { return true; }
  return false;
}
